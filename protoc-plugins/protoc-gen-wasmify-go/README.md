...
Place the wasm artifact (`<package>.wasm`, e.g. `mylib.wasm`) alongside the generated Go files so `go build` can resolve the embed.
...
`Init()` is the only entry point: there is no runtime override for the embed path. Test fixtures and CI must publish the wasm artifact under the correct filename before the package is built.
