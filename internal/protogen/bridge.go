...
func writeHeader(b *strings.Builder, spec *apispec.APISpec, packageName string, projectRoot string,
	...
	// Memory management. size_t keeps the export signature i32 on wasm32
	// (unchanged ABI) while widening it to i64 on wasm64, matching the
	// pointer width the host allocates with.
	b.WriteString("WASM_EXPORT(wasm_alloc)\nvoid* wasm_alloc(size_t size) { return malloc(size); }\n\n")
	b.WriteString("WASM_EXPORT(wasm_free)\nvoid wasm_free(void* ptr) { free(ptr); }\n\n")

	// Callback import. void*/size_t widen to i64 on wasm64 the same way.
	b.WriteString("WASM_IMPORT(wasmify, callback_invoke)\n")
	b.WriteString("int64_t wasmify_callback_invoke(int32_t callback_id, int32_t method_id, void* req, size_t req_len);\n\n")
}
...
// A bridge result crosses the boundary as one i64. On wasm32 it packs
// (ptr << 32) | len. On wasm64 pointers can exceed 32 bits, so the i64
// is instead the ADDRESS of a {payload_ptr, payload_len} u64 pair; the
// payload buffer still transfers to the reader, who frees it with
// wasm_free exactly as on wasm32, while the descriptor belongs to the
// returning side (a static pair here — bridge calls are serialized by
// the host-side lock). Zero means "no response" on both targets. The
// host runtime and decode_result below branch the same way, so one
// generated source builds correctly for either target.
#ifdef __wasm64__
static uint64_t wasmify_result_desc[2];
static int64_t encode_result(void* ptr, int32_t len) {
    wasmify_result_desc[0] = static_cast<uint64_t>(reinterpret_cast<uintptr_t>(ptr));
    wasmify_result_desc[1] = static_cast<uint64_t>(static_cast<uint32_t>(len));
    return static_cast<int64_t>(reinterpret_cast<uintptr_t>(wasmify_result_desc));
}
static void decode_result(int64_t rc, uintptr_t* ptr, int32_t* len) {
    if (rc == 0) { *ptr = 0; *len = 0; return; }
    const uint64_t* d = reinterpret_cast<const uint64_t*>(static_cast<uintptr_t>(rc));
    *ptr = static_cast<uintptr_t>(d[0]);
    *len = static_cast<int32_t>(d[1]);
}
#else
static int64_t encode_result(void* ptr, int32_t len) {
...
static void decode_result(int64_t rc, uintptr_t* ptr, int32_t* len) {
    *ptr = static_cast<uintptr_t>(static_cast<uint64_t>(rc) >> 32);
    *len = static_cast<int32_t>(rc & 0xFFFFFFFFu);
}
#endif
...
func writeTrampolineMethod(b *strings.Builder, m *apispec.Function, methodID int) {
	...
	b.WriteString("        uintptr_t _resp_ptr;\n")
	b.WriteString("        int32_t _resp_len;\n")
	b.WriteString("        decode_result(_rc, &_resp_ptr, &_resp_len);\n")
...
func GenerateBridgeHeader(spec *apispec.APISpec, packageName string) string {
	...
	b.WriteString("#include <cstdint>\n\n")
	b.WriteString("extern \"C\" {\n\n")
	b.WriteString("void* wasm_alloc(size_t size);\n")
